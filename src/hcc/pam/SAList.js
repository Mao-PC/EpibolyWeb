			...
			<div>
				...
				<div className="list-table">
					<Table
						...
						// scroll={{ x: 10 }}
						// expandedRowRender={(record, index) => this.getCountyHealth(record, index)}
						expandedRowRender={(record, index) => <SubTable id={record.orgId} />}
					/>
				</div>
			</div>
		);
		...
		return (
			<div className="list-table">
				<Table
					...
					// scroll={{ x: 10 }}
					pagination={false}
					expandedRowRender={(record, index) => <GrandTable id={record.orgId} />}
				/>
			</div>
		);
	}
